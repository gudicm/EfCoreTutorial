...
namespace EFCoreDatabaseFirstSample.Models.DataManager
{
    public class BookDataManager : IDataRepository<Book, BookDto>
    {
        ...
        public IEnumerable<BookShortDto> GetAllSelect()
        {
            var sBooks = _bookStoreContext.Book.Select(b => new BookShortDto() {Title = b.Title, Category = b.Category.Name}).ToList();
            return new List<BookShortDto>( );
        }
